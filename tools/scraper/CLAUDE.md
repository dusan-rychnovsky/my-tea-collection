...
- `domain/` — `TeaInfo`, `TeaType`, `Vendor`, plus opaque-type wrappers `Title`, `Name`, `Description` (over `String`), `Elevation` (over `Int`), `Location` (over `::[String]`, i.e. non-empty `List[String]`). Each lives in its own file. Pure data. No dependency on `parser`. Rendering helpers (`renderTeaInfo`, `renderTeaType`, `renderVendor`) live in `domain/TeaInfo.scala` for now; they'll move to a dedicated `render` subpackage when a second rendering format is added.
- `parser/` — `Fetcher` (co-locates `HttpError`), `HtmlParsing` (co-locates `ParseError` + shared JSoup helpers `parseElementText`, `parseAttributeText`, `parseElement`, `cleanText`), `Meileaf`, `Meetea`, `Scrape` (co-locates `UnsupportedVendorError`), `TeaTypeVocabulary` (co-locates `lookupTeaType` + its ZIO wrapper `resolveTeaType`), `LocationParsing` (co-locates `parseLocation` — splits a comma-separated origin string into a `Location`). HTTP fetch, HTML parsing, URL → parser routing. Depends on `domain`. Each error type is its own small case class living next to the function(s) that produce it; `ParseError` lives in `HtmlParsing` because the shared element-extraction helpers are its primary producers (the vendor parsers and `resolveTeaType` produce it too).
...
- **Optional fields**: `TeaInfo.cultivar/season` are `Option[String]`; `origin` is `Option[Location]`; `elevation` is `Option[Elevation]`. Different vendors surface different fields (e.g. meetea has no elevation). `renderTeaInfo` skips `None` lines and formats `Elevation` as `"{n}m"`, `Location` as comma-separated parts in **most-specific-first order** (reversed from the internal least-specific-first storage, so output matches the original page wire format). Other fields are required; `description` (a `Description`), `price`, `brewingInstructions` may be the literal string `"N/A"` (wrapped in `Description` where the type demands it) until parsing is implemented.
- **Opaque-type wrappers in `domain/`** for fields that would otherwise be primitives: `Title`, `Name`, `Description` (over `String`), `Elevation` (over `Int`), `Location` (over `::[String]`, i.e. non-empty `List[String]`), plus the structured `URL` from `zio.http` for `TeaInfo.url`. Each opaque type lives in its own file under `domain/`, has an `apply` smart constructor and a `value` extension method returning the underlying type. They currently perform no validation beyond what the type itself encodes (e.g. `Location` enforces non-emptiness via `apply(head: String, more: String*)`; a separate `Location.fromList(List): Option[Location]` lifts runtime lists). Constructors are explicit (`Title("…")`, `Elevation(900)`, `Location("China", "Fujian", "Fuding")` — note **least-specific first**, opposite the page wire format), no implicit conversions. To add a new wrapper, copy `domain/Title.scala`. Don't put richer validation into these until there's a real second case forcing it. `price` and `brewingInstructions` stay as `String` for now because they're placeholders; promote them when parsing lands.
- **String → typed-value parsing lives next to its only caller, then lifts to a shared file when a second vendor needs it.** Cleaning a raw HTML value into a typed domain value (e.g. `"900m approx"` → `Elevation(900)`, or `"Fuding, Fujian, China"` → `Location("China", "Fujian", "Fuding")`) is a parser-layer concern. Single-vendor helpers stay private at the top of that vendor's file (`parseElevation` in `parser/Meileaf.scala` — only meileaf surfaces elevation). Cross-vendor helpers get their own file: `parseLocation` lives in `parser/LocationParsing.scala` because both Meileaf and Meetea pull comma-separated origin strings. `parseLocation` reverses the wire order on read; `renderTeaInfo` reverses it again on write — so wire I/O is symmetric but internal storage stays canonical (least-specific-first). Don't preemptively put `parse` factories on the domain opaque types — keep them as nominal wrappers; parsing concerns can grow on the parser side.
- **Parsing helpers surface vendor weirdness rather than silently swallowing it.** Don't `.filter(_.nonEmpty)` after splitting on a separator: a blank field on the page (e.g. `"Fuding, Fujian, "`) parses to `Location("", "Fujian", "Fuding")` and renders with a visible trailing comma. The user has asked to be alerted to typos/missing-data on vendor sites, not have them disappear. `parseLocation` only returns `None` when the input is purely separators (Java's `split` returns an empty array), which is unambiguously "nothing there."
