...
- **Always review every finished change with an independent sub-agent.** Once a change is complete and green, spin up a separate review sub-agent to inspect the diff for introduced bugs, security vulnerabilities, test coverage (both unit and integration — every changed code path should be exercised), and overall code quality — style, design, and whether the code is idiomatic for the stack. Surface its findings and address them before considering the change done.
